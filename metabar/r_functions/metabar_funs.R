...
pbar <- function(ps, taxrank,
                 ...
                 sort_by_abund = TRUE,
                 cols = NULL) {
  #taxrank="phylum"; abund_tres=0.01; cols=mycols; ps=ps_prop
  #na_to_unknown = TRUE; sort_by_abund = TRUE; xvar = "time"; facetvar="group"
  ...
  df <- abund_stats(ps = ps, taxrank = taxrank, abund_tres = abund_tres,
                    na_to_unknown = na_to_unknown, sort_by_abund = sort_by_abund,
                    groupby = xvar, groupby2 = facetvar)
...
abund_stats <- function(ps, taxrank,
  ...
  if (!is.null(groupby)) {
    ...
    if (!is.null(groupby2)) {
      df <- df %>%
        dplyr::group_by(.data[[taxrank]], .data[[groupby]], .data[[groupby2]]) %>%
  ...
  if (!is.na(abund_tres)) {
    ...
    other <- df %>% filter(.data[[taxrank]] %in% to_lump)
    if (!is.null(groupby)) {
      if (is.null(groupby2)) other <- other %>% dplyr::group_by(.data[[groupby]])
      if (!is.null(groupby2)) other <- other %>% dplyr::group_by(.data[[groupby]],
                                                                .data[[groupby2]])
      other <- other %>% summarize(Abundance = sum(Abundance))
