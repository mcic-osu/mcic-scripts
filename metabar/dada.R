...
regex_remove <- "(_S\\d\\d)?_L00\\d_R1_.*"  # Remove this part from the FASTQ filename to get sampleIDs
...
parser$add_argument("--n_samples",
                    ...
                    help = "Number of samples to analyze [default: 0 => all]")
parser$add_argument("--start_at_step",
                    type = "integer", default = 1,
                    help = "Start at step n")
...
start_at_step <- as.integer(args$start_at_step)
...
fq_raw_f <- sort(list.files(indir, pattern = "_R1.*fa?s?t?q.*", full.names = TRUE))
fq_raw_r <- sort(list.files(indir, pattern = "_R2.*fa?s?t?q.*", full.names = TRUE))

# If needed, select only a subset of the FASTQ files
n_samples_all <- length(fq_raw_f)
if (n_samples == 0) n_samples <- n_samples_all
fq_raw_f <- fq_raw_f[1:n_samples]
fq_raw_r <- fq_raw_r[1:n_samples]

# Extract sample IDs from FASTQ file names
sample_ids <- sub(regex_remove, "", basename(fq_raw_f))
...
message("# ====================================================================")
Sys.time()
message("# Dir with input FASTQ files:             ", indir)
...
message()
if (! is.null(config_file)) message("# For settings, used config file:          ", config_file)
message("# ====================================================================")
message()
...
if (start_at_step <= 1) {
    ...
    saveRDS(filt_res, file.path(rds_dir, "filt_res.rds"))
}
...
if (start_at_step <= 2) {
    ...
    # Save objects to RDS files
    saveRDS(fq_derep_f, file.path(rds_dir, "fq_derep_f.rds"))
    saveRDS(fq_derep_r, file.path(rds_dir, "fq_derep_r.rds"))
}
...
if (start_at_step <= 3) {
    ...
    # Save objects to RDS files
    saveRDS(err_f, file.path(rds_dir, "err_f.rds"))
    saveRDS(err_r, file.path(rds_dir, "err_r.rds"))
...
if (start_at_step <= 4) {
    ...
    # Save objects to RDS files
    saveRDS(dada_f, file.path(rds_dir, "dada_f.rds"))
    saveRDS(dada_r, file.path(rds_dir, "dada_r.rds"))
}
...
if (start_at_step <= 6) {
    ...
    # Save objects to RDS files
    saveRDS(seqtab_all, seqtab_all_file)
}
...
if (start_at_step <= 8 & !(asv_size_min == 0 & asv_size_max == Inf)) {
    ...
    # Save objects to RDS files
    saveRDS(seqtab_lenfilt, seqtab_lenfilt_file)
...
if (!is.null(techrep_file)) {
    ...
    sum_dup <- function(sampleid_dup, seqtab, dup_df) {
        ...
        dup_sum_row <- t(as.data.frame(colSums(seqtab[dup_idxs, ], na.rm = TRUE)))
...
nasv_summary <- data.frame(step = c("initial", "non_chimeric", "lenfilter"),
                           n_asv = c(ncol(seqtab_all), ncol(seqtab_nochim), ncol(seqtab_lenfilt)))
